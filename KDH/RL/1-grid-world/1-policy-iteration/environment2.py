...
class GraphicDisplay(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        canvas.create_window(WIDTH * UNIT * 0.13, HEIGHT * UNIT + 16, window=iteration_button)
        
        policy_button = Button(self, text="Improve", command=self.improve_policy)
        policy_button.configure(width=10, activebackground="#33B5E5")
        canvas.create_window(WIDTH * UNIT * 0.37, HEIGHT * UNIT + 16, window=policy_button)

        policy_button = Button(self, text="move", command=self.move_by_policy)
        policy_button.configure(width=10, activebackground="#33B5E5")
        canvas.create_window(WIDTH * UNIT * 0.62, HEIGHT * UNIT + 16, window=policy_button)

        policy_button = Button(self, text="reset", command=self.reset)
        policy_button.configure(width=10, activebackground="#33B5E5")
        canvas.create_window(WIDTH * UNIT * 0.87, HEIGHT * UNIT + 16, window=policy_button)
    ...
    def render(self):
        time.sleep(0.1)
        self.canvas.tag_raise(self.rectangle) # 맨 위로 올리기
        self.update()
